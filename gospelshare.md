...
- Persistent config file (`.gospelshare`) to save default settings with `--saveconfig`
...
gospelshare reads defaults from a `.gospelshare` file in the current directory. Command-line arguments always override the config file.
...
This writes a `.gospelshare` file like:
```
# gospelshare configuration — generated by gospelshare --saveconfig
bv          = BSB
...
- `.gospelshare` - Optional config file for persistent defaults (created by `--saveconfig`)
